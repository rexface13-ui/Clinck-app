...
# ── [1/5] الباك إند بنافذة منفصلة ────────────────────────────────────────
Write-Host "[1/5] Starting backend server (port 8010)..."
...
# ── [2/5] الواجهة (خادم PHP الثابت مع الـ router اللي بيمرّر /api للباك إند) ──
Write-Host "[2/5] Starting frontend server (port 5183)..."
...
# ── [3/5] بوت تيليغرام بنافذة منفصلة — لو التوكن مش معبّى بعد، الأمر بيطبع
#          تحذير ويرجع فوراً بدون ما يعلّق النافذة (مش خطأ يوقف التشغيل).
Write-Host "[3/5] Starting Telegram bot listener..."
Start-Process -FilePath "cmd.exe" -ArgumentList "/k", "title DentaFlow - Telegram Bot && cd /d `"$BACKEND`" && `"$PHP`" artisan telegram:poll"

# ── [4/5] المجدول — بدونه ما بينبعت أي تذكير ولا تقرير يومي، لأن الأوامر
#          المجدولة بدها حدا يفحصها كل دقيقة.
Write-Host "[4/5] Starting scheduler..."
Start-Process -WindowStyle Minimized -FilePath "cmd.exe" -ArgumentList "/k", "title DentaFlow - Scheduler && cd /d `"$BACKEND`" && `"$PHP`" artisan schedule:work"
...
Write-Host ""
Write-Host "  Close the 'DentaFlow - Backend', 'DentaFlow - Frontend'," -ForegroundColor Green
Write-Host "  'DentaFlow - Telegram Bot', and 'DentaFlow - Scheduler'" -ForegroundColor Green
