...
الاختبارات بتشتغل على **Postgres** مش sqlite — لأنهم بيختلفوا بأشياء بتهمنا هون: خطأ
`column reference "created_at" is ambiguous` اللي وقع بالإنتاج بيظهر بـ Postgres بس،
وsqlite بيمرّره بصمت. مجموعة اختبارات بتنجح على sqlite بينما الإنتاج بينكسر أسوأ من
لا اختبارات أصلاً.

قاعدة اختبارات منفصلة اسمها `dentaflow_test` لازم تكون موجودة (بتتعمل مرة وحدة):

```bash
createdb -U postgres dentaflow_test
```
...
التغطية الحالية بتركّز على المسارات المالية: التحصيل، الخصومات، حذف الفواتير، الشيكات
وارتجاعها، إلغاء الشغل وعكس العمولة، وأرقام الفواتير/أكواد المرضى المتسلسلة. كل اختبار
موثّق بالخطأ اللي انكتب عشانه.
